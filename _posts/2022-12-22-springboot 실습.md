...
원래 html, css 코드를 작성할 때 접속하는 기기의 화면 크기에 따라 형태가 무너지지 않도록 신경써줘야 하지만, 부트스트랩에서 반응형으로 제공되므로 그런 신경을 써줄 필요가 없다는 점이 편리했다.
...
flex 배치로 화면을 다음처럼 구성했다. 앞으로 계속 배워나가며 더 효율적인 배치를 고민해보고 싶다.
